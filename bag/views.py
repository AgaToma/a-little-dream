...
def view_bag(request):
    ...
    for users to view the contents of their bag
    """
...
def add_to_bag(request, item_id):
    """
    Add a quantity of the specified product to bag
    """
...
def adjust_bag(request, item_id):
    """
    Update quantity of a specified product
    """
    ...
    if quantity > 0:
        if quantity > product.stock_level:
            messages.error(request, f'Sorry, but we only have \
                { product.stock_level } of { product.name } at the moment. \
